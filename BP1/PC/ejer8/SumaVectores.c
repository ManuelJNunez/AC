...
void Inicializa(double vector1[], double vector2[], int ini, int fin, int N){
    for(int i=ini; i<fin; i++){ 
      vector1[i] = N*0.1+i*0.1; 
      vector2[i] = N*0.1-i*0.1;
    }
}

void Suma(double vector1[], double vector2[], double vector3[], int ini, int fin){
    for(int i=ini; i<fin; i++)
      v3[i] = v1[i] + v2[i];
}
...
int main(int argc, char** argv){ 
 ...
 #pragma omp parallel sections
  {
    #pragma omp section
    Inicializa(v1, v2, 0, N/4, N);

    #pragma omp section
    Inicializa(v1, v2, N/4, N/2, N);

    #pragma omp section
    Inicializa(v1, v2, N/2, 3*N/4, N);

    #pragma omp section
    Inicializa(v1, v2, 3*N/4, N, N);
  }
  ...
  #pragma omp parallel sections
  {
    ...
    #pragma omp section
    Suma(v1, v2, v3, N/4, N/2);

    #pragma omp section
    Suma(v1, v2, v3, N/2, 3*N/4);

    #pragma omp section
    Suma(v1, v2, v3, 3*N/4, N);
  }
  ...
  for(i=0; i<N; i++) 
	...
	printf("Primer componente v1: %11.9f y última: %11.9f\n",v1[0],v1[N-1]);
	printf("Primer componente v2: %11.9f y última: %11.9f\n",v2[0],v2[N-1]);
	printf("Primer componente v3: %11.9f y última: %11.9f\n",v3[0],v3[N-1]);
